	...
	public function getUltimosAnuncios($page, $perPage, $filtros) {
		...
		$sql = $pdo->prepare("SELECT
			*,
			...
			FROM anuncios WHERE ".implode(' AND ', $filtrostring)." ORDER BY id DESC LIMIT $perPage");
		...
		if(count($fotos) > 0) {
			for($q=0;$q<count($fotos['tmp_name']);$q++) {
				...
				if(in_array($tipo, array('image/jpeg', 'image/png'))) {
					...
					$sql = $pdo->prepare("INSERT INTO anuncios_imagens (id_anuncio, url) VALUES (:id_anuncio, :url)");
					$sql->bindValue(":id_anuncio", $id);
